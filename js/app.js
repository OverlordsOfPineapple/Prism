...
function scheduleBackdropPreload(url){const connection=navigator.connection||navigator.mozConnection||navigator.webkitConnection;if(connection?.saveData||/2g/.test(connection?.effectiveType||''))return;const run=()=>{const image=new Image();image.decoding='async';image.fetchPriority='low';image.src=url};if('requestIdleCallback' in window)requestIdleCallback(run,{timeout:3000});else setTimeout(run,1800)}
function setHero(i){state.hero=(i+featuredMovies.length)%featuredMovies.length;const m=featuredMovies[state.hero],media=$('#heroMedia'),firstPaint=media.dataset.ready!=='1';clearTimeout(setHero.timer);const applyBackdrop=()=>{if(media.getAttribute('src')!==m.backdrop)media.src=m.backdrop;media.style.opacity='1';media.dataset.ready='1';scheduleBackdropPreload(featuredMovies[(state.hero+1)%featuredMovies.length].backdrop)};if(firstPaint){applyBackdrop()}else{media.style.opacity='0';setHero.timer=setTimeout(applyBackdrop,90)}const content=$('#heroContent');if(!firstPaint){content.classList.remove('animate');requestAnimationFrame(()=>content.classList.add('animate'))}$('#heroTitle').textContent=m.title.toUpperCase();$('#heroMeta').textContent=`IMDb ${m.imdb} · ${m.year} · ${m.runtime} · ${m.classification} · ${m.genre}`;$('#heroOverview').textContent=m.overview;$$('.hero-dot').forEach((d,n)=>d.classList.toggle('active',n===state.hero));$('#heroInfo').onclick=()=>openModal(m.id);$('#playHero').onclick=()=>openTrailer(m)}
...
const providerHomepages={'ABC iview':'https://iview.abc.net.au/','7plus':'https://7plus.com.au/','9Now':'https://www.9now.com.au/','SBS On Demand':'https://www.sbs.com.au/ondemand/','10':'https://10.com.au/','Netflix':'https://www.netflix.com/au/','Disney+':'https://www.disneyplus.com/en-au','Prime Video':'https://www.primevideo.com/','Binge':'https://binge.com.au/','Stan':'https://www.stan.com.au/','Apple TV+':'https://tv.apple.com/au/'};
const freeTvAliases={
  'ABC iview':'ABC iview','ABC iView':'ABC iview','ABC':'ABC iview',
  '7plus':'7plus','7Plus':'7plus','Seven':'7plus',
  '9Now':'9Now','Nine':'9Now',
  'SBS On Demand':'SBS On Demand','SBS':'SBS On Demand',
  '10':'10','10 Play':'10','10 play':'10','Network 10':'10'
};
const freeTvNames=new Set(['ABC iview','7plus','9Now','SBS On Demand','10']);
function providerName(p){return typeof p==='string'?p:p?.name||''}
function canonicalProviderName(name){return freeTvAliases[name]||name}
function providerMatches(p,service){return canonicalProviderName(providerName(p))===canonicalProviderName(service)}
function freeTvProviders(m){return [...(m.providers?.free||[]),...(m.providers?.ads||[])].filter((p,i,a)=>a.findIndex(x=>providerName(x)===providerName(p))===i)}
function isFreeTvMovie(m){return freeTvProviders(m).length>0}

function providerCard(p,label='Stream',watchLink=''){const name=providerName(p),canonical=canonicalProviderName(name);const logo=typeof p==='string'?'':p.logo;const href=providerHomepages[canonical]||watchLink||providerHomepages[name]||'';const content=`${logo?`<img src="${logo}" alt="" loading="lazy" decoding="async" fetchpriority="low" width="84" height="84">`:`<span aria-hidden="true">${name.slice(0,1)}</span>`}<div><strong>${name}</strong><small>${label} in Australia</small></div>`;return href?`<a class="provider-card" href="${href}" target="_blank" rel="noopener noreferrer" aria-label="Open ${name} availability">${content}</a>`:`<article class="provider-card">${content}</article>`}
...
function previewBackdrop(id){const m=movies.find(x=>x.id===Number(id));if(!m||state.view!=='home')return;const media=$('#heroMedia');if(media.getAttribute('src')!==m.backdrop)media.src=m.backdrop;}
...
async function initialiseLiveData(){
  ...
  return loadLiveCatalogue({showDialogOnFailure:false,showToast:false});
}
function scheduleInitialLiveData(){
  const start=()=>{const run=()=>initialiseLiveData();if('requestIdleCallback' in window)requestIdleCallback(run,{timeout:2500});else setTimeout(run,750)};
  if(document.readyState==='complete')start();
  else window.addEventListener('load',start,{once:true});
}

scheduleInitialLiveData();
