...
function setHero(i){state.hero=(i+featuredMovies.length)%featuredMovies.length;const m=featuredMovies[state.hero],media=$('#heroMedia');const firstPaint=!media.style.backgroundImage;media.style.opacity=firstPaint?'1':'0';clearTimeout(setHero.timer);const applyBackdrop=()=>{media.style.backgroundImage=`url('${m.backdrop}')`;media.style.opacity='1';scheduleBackdropPreload(featuredMovies[(state.hero+1)%featuredMovies.length].backdrop)};if(firstPaint)applyBackdrop();else setHero.timer=setTimeout(applyBackdrop,90);const content=$('#heroContent');content.classList.remove('animate');requestAnimationFrame(()=>content.classList.add('animate'));$('#heroTitle').textContent=m.title.toUpperCase();$('#heroMeta').textContent=`IMDb ${m.imdb} · ${m.year} · ${m.runtime} · ${m.classification} · ${m.genre}`;$('#heroOverview').textContent=m.overview;$$('.hero-dot').forEach((d,n)=>d.classList.toggle('active',n===state.hero));$('#heroInfo').onclick=()=>openModal(m.id);$('#playHero').onclick=()=>openTrailer(m)}
